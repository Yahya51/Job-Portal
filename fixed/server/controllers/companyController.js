...
export const registerCompany = async (req, res) => {
    ...
    if (!name || !email || !password || !imageFile) return res.json({ success: false, message: "Missing Details" })
    try {
        const companyExists = await Company.findOne({ email })
        if (companyExists) return res.json({ success: false, message: "Company already registered" })
...
export const postJob = async (req, res) => {
    ...
    try {
        const newJob = new Job({ title, description, location, salary, companyId, date: Date.now(), category, level })
        await newJob.save()
...
export const getCompanyPostedJobs = async (req, res) => {
    try {
        ...
        // FIX: was returning `jobs` instead of `jobsData` — applicant count not attached
